...
//
//     if (dirs.length === 0) {
//         console.error('エラー: ディレクトリが指定されていません。');
//         console.error('使用例: node cleanup.js ./build/ ./dist/');
//         process.exit(1);
//     }
//
//     console.log('🔍 指定されたディレクトリ:', dirs);
//
//     // すべてのJSファイルを収集
//     const allFiles = new Set();
//
...
//
//             if (!stats.isDirectory()) {
//                 console.error(`💥 エラー: "${dir}" はディレクトリではありません。`);
//                 continue;
//             }
//
//             console.log(`📁 "${dir}" からJSファイルを検索中...`);
//
...
//
//         } catch (error) {
//             console.error(`💥 エラー: ディレクトリ "${dir}" の処理中に問題が発生しました。`);
//             console.error(`詳細: ${error.message}`);
//         }
//     }
//
...
//
//     if (fileList.length === 0) {
//         console.warn('⚠️  警告: JSファイルが見つかりませんでした。');
//         return;
//     }
//
//     console.log(`\n📋 ${fileList.length}個のJSファイルをクリーンアップします:`);
